...
@Repository("lmsPortalFeedbackQueryDAO")
public class LmsPortalFeedbackQueryDAO extends BaseDAO<PortalFeedbackQuery> {
	...
	public void getLmsDb(LmsDb bean) {
		tableName = bean.getLmsDb() + ".portal_feedback_query";
	...
	public List<PortalFeedbackQuery> findQueryList(Long portalFeedbackResponseId) {
		String sql = "select pfq.id, pfq.answer,pfq.parentId,pfq.createdBy as createdBy, pfq.createdDate from "+ getTableName() +" pfq where pfq.parentId=?";
	...
	public List<PortalFeedbackQuery> findQueryByPortalFeedbackId(
		...
		String sql = "select id,portalFeedbackId,answer,createdBy,createdDate,parentId from "
				+ getTableName() + " where portalFeedbackId = ? and answer <> '' ";
