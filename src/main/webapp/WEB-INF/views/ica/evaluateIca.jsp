        ...
        <div class="newLoader">
				...
				<div
					...
					<div class="card bg-white border" id="manual">
						...
						<div class="card-body">
							<form id="icaGrade"
								action="${pageContext.request.contextPath}/submitIca"
								method="post">
												...
												<c:when test="${icaQuery eq 'true'}">
													<c:choose>
														...
														<c:when test="${ica.scaledReq eq 'Y'}">
															...
															<input type="hidden" id="scaledM"
																value="${ica.scaledMarks}" />
														</c:when>
														<c:otherwise>
													...
													<c:forEach var="stud" varStatus="statusStud"
														...
														<c:if test="${ica.scaledReq eq 'Y'}">
															...
															<td><input class="text-center marks" type="text"
																placeholder="Scaled Marks" readonly="readonly"
																id="scale${stud.username}" name="scale${stud.username}"
																value="${studentMarksMap[keyScale]}"></td>
														</c:if>
														<c:if test="${icaQuery eq 'true'}">
														<td>
												...
												<c:otherwise>
													...
													<c:forEach var="stud" varStatus="statusStud"
														...
														id="isAbsentN" style="margin-right: 4px;" onclick="enableTr('tr${statusStud.count}');" class="absentStatus">No
															...
															<td><c:set var="keyTotal"
																	value="${stud.username}totalM" /> <c:set
																	var="keyRemark" value="${stud.username}remark" /> <input
																class="text-center marks" type="text"
																placeholder="Total Marks" readonly="readonly"
																id="total${stud.username}" name="total${stud.username}"
																value="${studentMarksMap[keyTotal]}"></td>
															<c:if test="${ica.scaledReq eq 'Y'}">
																<c:set var="keyScale" value="${stud.username}scaleM" />
																<td><input class="text-center marks" type="text"
																	placeholder="Scaled Marks" readonly="readonly"
					...
					<div class="card bg-white border" id="excel">
						<div class="card-body">
							...
							<form:form id="uploadIcaFromExcel"
								action="uploadStudentMarksExcel" method="post"
								...
								<div class="col-lg-4 col-md-4 col-sm-12 mt-3 position-relative">
									...
									<button id="submit" class="btn btn-secondary mt-2"
										formaction="uploadStudentMarksExcel?saveAs=draft&icaId=${icaBean.id}"
										onclick="return confirm('Are you sure you want to save?')">Save
										As Draft</button>
									<button id="submit" class="btn btn-dark mt-2"
										formaction="uploadStudentMarksExcel?saveAs=submit&icaId=${icaBean.id}"
					...
					$(".componentMarks")
							.change(
									function() {
										...
										total = performtotalCalc(splitId[0]);
										$('#total' + splitId[0]).val(total);
