			...
			<div class="row">
									...
									<div class="row">
										<div class="mobTimerW col-12">
											...
											<div class="col-12 m-auto" id="CountDownTimerMobile"
												style="width: 250px;" data-timer="${testTime}"></div>
										</div>
										<div
													...
													<form:form action="addStudentQuestionResponse"
														...
														<ul class="list-unstyled">
															...
															<li><c:choose>
																	<c:when
																		...
																		<p class="bg-blue subHead1 text-white p-3"
																			id="testQuest">${testQuestion.description}</p>
																	</c:when>
																	<c:otherwise>
																		<c:choose>
																			<c:when test="${testQuestion.type eq 'SINGLESELECT'}">
																				<p class="bg-blue subHead1 text-white p-3"
																					id="testQuest">Select Any One Of the Following
																					Options: ${testQuestion.description}</p>
																			</c:when>
																			<c:otherwise>
																				<p class="bg-blue subHead1 text-white p-3"
																					id="testQuest">Select Multiple From Following
																					Options: ${testQuestion.description}</p>
																			</c:otherwise>
																		</c:choose>
																	</c:otherwise>
															...
															<c:choose>
																...
																<c:when test="${testQuestion.questionType eq 'Numeric'}">
																...
																<div class="row">
																	<c:if test="${not status.first}">
																		<%-- <div
																			...
																			<button type="button" class="w-100 btn-next"
																				id="prevQuest"
																				onclick="showModal('modal-${status.index }','modal-${status.index - 1}','nav-${status.index - 1}')">PREVIOUS</button>
																		</div> --%>
																		<div
																	...
																	<c:choose>
																		<c:when test="${status.last}">
																			<button type="button" class="w-100 btn-next"
																				id="nextQuest"
																				onclick="submitForm('studentTestForm-${status.index}','nav-${status.index}');showModal('modal-${status.index }','modal-added','nav-${status.index + 1}')">SAVE AND COMPLETE</button>
																		</c:when>
																		<c:otherwise>
																			<div
				...
				<script>
					...
					window.showModalOnClick = function showModalOnClick(
						...
						$(".modalClass").each(function() {
							...
							if (id == modalToShow) {
								/* console
										.log('funcion showmodalclick called');
								$('#' + btnId).addClass(
										'activeQbtn').siblings(
										'.activeBtnClass')
										.removeClass('activeQbtn'); */
				...
				<script>
					...
					function completeTest(){
                        ...
                        console.log('complete test called ajax'+studentTestId);
                        return $
                        .post(
                                    'completeStudentTestAjax',
                                    {
                                          studentTestId : studentTestId,
					...
					window.saveStudentsTest = function saveStudentsTest(
						...
						$
								.ajax({
									type : 'POST',
									url : '${pageContext.request.contextPath}/completeStudentTestAjax?'
											+ 'studentTestId=' + studTestId
											+'&studentFilePath='+$('#studentFilePath').val(),
									success : function(data) {
