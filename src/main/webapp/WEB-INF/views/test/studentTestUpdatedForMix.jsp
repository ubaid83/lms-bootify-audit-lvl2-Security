												...
												<div class="col-12 modalClass" id="modal-${status.index}">
													...
													<form:form action="addStudentQuestionResponse"
														...
														<form:input type="hidden"
															path="testQuestions[${status.index}].studentQuestionResponse.attemptNo"
															value="${test.studentTest.attempt}" />
														<form:input type="hidden"
														...
														<ul class="list-unstyled">
															...
															<li>
																<p class="bg-blue subHead1 text-white p-3"
																	id="testQuest">
																<h3>Marks: ${testQuestion.marks}</h3>${testQuestion.description}</p>
															...
															<li>
																<ul class="ml-4 list-unstyled moduleTestAns">
																	...
																	<c:choose>
																			...
																			<c:choose>
																				...
																				<c:when
																					...
																					<form:textarea rows="1" cols="20"
																						...
																						id="checkField${status.count}1"
																						onkeypress="return isNumberKey(event)"
																						style="margin-top: 10px;margin-bottom:10px resize: none;" />
																				</c:when>
																														...
																														<c:choose>
																															<c:when
																																...
																																<form:radiobutton
																																	path="testQuestions[${status.index}].answer"
																																	value="4"
																																	id="checkField${status.count}4"
																																	onclick="submitForm('studentTestForm-${status.index}','nav-${status.index}');checkFields('checkField${status.count}1','label${status.count}4',true)" />
																															</c:when>
																															<c:otherwise>
																																<form:checkbox
																																	path="testQuestions[${status.index}].answers"
																		...
																		<%-- 	<c:otherwise>
																			<c:choose>
																				<c:when test="${'Y' eq testQuestion.optionShuffle}">
																					<%
																						Random rand = new Random();
																												Integer n = rand.nextInt(3) + 1;
																												if (n == 1) {
																									...
																									<div class="input-group-text">
																										<c:choose>
																											...
																											<c:otherwise>
																												<form:checkbox
																													path="testQuestions[${status.index}].answers"
																													value="2" id="checkField${status.count}5"
																													onclick="submitForm('studentTestForm-${status.index}','nav-${status.index}');checkFields('checkField${status.count}5','label${status.count}5',false)" />
																											</c:otherwise>
																										</c:choose>
																									</div>
																									...
																									<div class="input-group-text">
																										<c:choose>
																											...
																											<c:otherwise>
																												<form:checkbox
																													path="testQuestions[${status.index}].answers"
																													value="2" id="checkField${status.count}7"
																													onclick="submitForm('studentTestForm-${status.index}','nav-${status.index}');checkFields('checkField${status.count}7','label${status.count}7',false)" />
																											</c:otherwise>
																										</c:choose>
																									</div>
												...
												<ul class="list-unstyled">
													...
													<li>
														...
														<p>
															...
															<span>Reattempt:</span><br> Allowed
															${test.maxAttempt} times
													</li>


													<li><sec:authorize access="hasAnyRole('ROLE_FACULTY','ROLE_ADMIN')">
															<a type="button" class="btn btn-modalClose"
																href="${pageContext.request.contextPath}/testList">Close</a>
														</sec:authorize> <sec:authorize access="hasRole('ROLE_STUDENT')">
															<a type="button" class="btn btn-modalClose"
																href="${pageContext.request.contextPath}/viewTestFinal?courseId=${test.courseId}">Close</a>
														</sec:authorize></li>
