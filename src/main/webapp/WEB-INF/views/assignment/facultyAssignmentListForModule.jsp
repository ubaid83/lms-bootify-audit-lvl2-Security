						...
						<div class="card-body">
							<div class="col-xs-12 col-sm-12">
								<div class="x_panel">
									...
									<form:form action="searchFacultyAssignmentForModule"
												...
												<div class="col-md-3 col-md-3 col-sm-12 column">
													<form:select id="moduleId" path="moduleId"
														...
														<c:if test="${ empty moduleId }">
															<form:option value="" selected="selected" disabled="true">Select Module</form:option>
														</c:if>
														<c:if test="${ not empty moduleId }">
															<form:option value="" disabled="true">Select Module</form:option>
														</c:if>
														<c:forEach var="module" items="${allModules}"
															varStatus="status">
															<c:if test="${ moduleId eq module.moduleId}">
																<form:option value="${module.moduleId}" selected="selected">${module.moduleName}</form:option>
															</c:if>
															<c:if test="${ moduleId ne module.moduleId}">
															...
															<tbody>
																...
																<c:forEach var="assignment" items="${assignmentList}"
																	...
																	<tr>
																		...
																		<td><c:out value="${assignment.moduleName}" /></td>
																		...
																		<%-- <c:choose>
																			<c:when
																				test="${assignment.allocateToStudents eq 'Y'}">
																				<td class="d-flex"><c:url
																						value="viewAssignmentForModule" var="viewUrl">
																						<c:param name="id" value="${assignment.id}" />
																					</c:url>
																			</c:when>
																			<c:otherwise>
																				<td class="d-flex"><c:url
																						value="viewAssignment" var="viewUrl">
																						<c:param name="id" value="${assignment.id}" />
																					</c:url>
																			</c:otherwise>
																		</c:choose> --%>
																		<td class="d-flex">
																		<c:url value="viewAssignmentForModule" var="viewUrl">
																			<c:param name="id" value="${assignment.id}" />
																		</c:url>
																		<c:url value="deleteAssignment" var="deleteurl">
																			<c:param name="id" value="${assignment.id}" />
																		</c:url>
																		<c:url value="searchAssignmentToEvaluateForModule"
																			var="evaluateAssignmentUrl">
																			<c:param name="id"
																				value="${assignment.id}" />
																			<c:param name="moduleId"
																				value="${assignment.moduleId}" />
																		</c:url>
																		<c:url value="viewByGroupAssignment"
																			var="viewByGroupUrl">
																			<c:param name="id" value="${assignment.id }"></c:param>

																		</c:url>
																		<a href="${viewUrl}" title="View Assignment"><i
																			class="fa fa-info-circle fa-lg"></i></a>&nbsp;
																		<sec:authorize access="hasAnyRole('ROLE_FACULTY')">
																			<a href="${evaluateAssignmentUrl}"
																		...
																		&nbsp; --%>
																		<%-- <a href="${viewByGroupUrl}" title="View By Group"><i
																			class="fa fa-users"></i></a>&nbsp; --%>
																		<sec:authorize access="hasAnyRole('ROLE_FACULTY')">&nbsp;<a
																				href="${deleteurl}"
																		...
																		<sec:authorize access="hasAnyRole('ROLE_ADMIN')">
																			...
																			<a href="${makeAssignmentInactiveurl}"
																				...
																				class="fa fa-check-square-o fa-lg"></i></a>&nbsp;</sec:authorize>
																		</td>
																	</tr>
																</c:forEach>
