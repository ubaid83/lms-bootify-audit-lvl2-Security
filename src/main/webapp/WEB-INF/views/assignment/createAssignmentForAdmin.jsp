...
<div class="d-flex adminPage" id="adminPage">
												...
												<form:select id="idForCourse" path="courseId"
													...
													<c:forEach var='cList' items='${allCourses}'>
														<c:if test="${courseId eq cList.id}">
															<option value="${cList.id}" selected>${cList.courseName} - (${cList.acadSession} ${cList.acadYear})</option>
														</c:if>
														<c:if test="${courseId ne cList.id }">
															<option value="${cList.id}">${cList.courseName} - (${cList.acadSession} ${cList.acadYear})</option>
														</c:if>
													</c:forEach>
												</form:select>
											</div>
 ...
 %>
							...
							<hr />
							<c:if test="${assignmentStarted}">
								<form:input type="hidden" path="isQuesConfigFromPool" value="${assignment.isQuesConfigFromPool}"/>
								<form:input type="hidden" path="randomQuestion" value="${assignment.randomQuestion}"/>
								<form:input type="hidden" path="maxQuestnToShow" value="${assignment.maxQuestnToShow}"/>
							</c:if>
							<c:if test="${!assignmentStarted}">
							...
							<div class="col-lg-4 col-md-4 col-sm-12">
								<label class="textStrong">Configure Question from pool?</label>
								<div>
									<div class="pretty p-switch p-fill p-toggle">
										<form:checkbox id="isQuesConfigFromPool"
											value="${ assignment.isQuesConfigFromPool }"
											path="isQuesConfigFromPool" />
										<div class="state p-success p-on">
											<label>Yes</label>
										</div>
										<div class="state p-danger p-off">
							...
							<div class="col-lg-4 col-md-4 col-sm-12">
								...
								<div>
									<div class="pretty p-switch p-fill p-toggle">
										<form:checkbox id="isQuesConfigFromPool" value="N"
											path="isQuesConfigFromPool" />
										<div class="state p-success p-on">
											<label>Yes</label>
										</div>
							...
							<div id="quesPool" style="display: none">
								<div class="col-lg-4 col-md-4 col-sm-12">
									<label class="textStrong">Random Question Required?</label>
									<div>
										<div class="pretty p-switch p-fill p-toggle">
											<form:checkbox id="randQReq"
												value="${ assignment.randomQuestion }" path="randomQuestion" />
											<div class="state p-success p-on">
												<label>Yes</label>
											</div>
											<div class="state p-danger p-off">
										...
										<div class="col-6 m-auto">
											<div class="form-row mt-5">
												...
												<button id="submit" onclick="myFunction()"
													class="btn btn-info col-md-5 col-sm-12  ml-auto mr-auto mb-3"
													formaction="addAssignmentByAdmin">Create Assignment</button>
												<%
													}
												%>
		...
		<script type="text/javascript">
					...
					$(document)
							.ready(
									function() {
										...
										/*  For Assignment Pool Start */
										let quesConfigFromPoolLoad = $("#isQuesConfigFromPool").is(":checked");
										if (quesConfigFromPoolLoad === true) {
											$('#quesPool').show();
											$("#inputMaxQ").parent().remove("d-none")
										}
										/* For Assignment Pool End */
									});
				</script>
				...
				<script>
					$(function() {
						...
						/*  For Assignment Pool Start */
						$("#isQuesConfigFromPool").change(function() {
							...
							if (isChecked === true) {
								...
								$('#quesPool').hide();
								$('#randQReq').val("N").prop("checked", false);
								$("#inputMaxQ").val(0).parent().addClass("d-none")
							}
						});
