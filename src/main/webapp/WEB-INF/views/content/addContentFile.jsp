				...
				<div
					...
					<div class="card bg-white border">
						<div class="card-body">
							...
							<h5 class="text-center pb-2 border-bottom">Add File for
								${courseIdNameMap[content.courseId] }</h5>
							...
							<form:form action="createAssignment" id="createAssignment"
								...
								<%-- 	<form:input path="acadMonth" type="hidden"/> --%>
									 ...
									 <c:if test="${content.parentModuleId ne null }">
										<form:input path="acadMonth" type="hidden"/>
								...
								<%-- 	<form:input type="hidden" path="moduleId" /> --%>
									...
									<div class="row">
										...
										<div class="col-md-6 col-sm-12">
											<div class="form-group">
												<form:label path="contentName" for="contentName"><strong>File Name </strong><span
														...
														<form:select id="idForCourse" path="idForCourse"
															...
															<c:forEach var="course" items="${courses}"
																...
																<c:if test="${content.courseId ne null}">
																	<c:if test="${content.courseId eq course.id}">
																		<option value="${course.id}" selected>${course.courseName}</option>
																	</c:if>
																	<c:if test="${content.courseId ne course.id}">
										...
										<div class="col-md-6 col-sm-12">
											...
											<div class="form-group">
												<form:label path="accessType" for="accessType"><strong>Access Type (Read Notes below) </strong><span
														style="color: red">*</span>
												</form:label>
												<%-- <c:if test="${ not empty content.accessType }">
															<form:hidden path="accessType" />
										...
										<div class="col-12">
											<div class="form-group">
												<form:label path="contentDescription"
													for="contentDescription"><strong>File Description</strong></form:label>
											...
											<div class="form-group">
												<form:label path="" for="file">
													<%
														if (isEdit) {
													%>
																<strong>Select File only if you wish to override earlier file </strong><i
														class="fa fa-file-o"></i>
													<%
														} else {
													%>
																<strong>Select File from Computer </strong><i class="fa fa-file-o"></i>
													<%
														}
													%>
										...
										<div class="col-md-6 col-sm-12 mt-3 position-relative">
											<label><strong>Select Date Range <span style="color: red">*</span></strong></label>
											<div class="input-group">
												<form:input type="hidden" id="testStartDate"
													path="startDate" />
												<form:input type="hidden" id="testEndDate" path="endDate" />
								 	...
								 	<sec:authorize access="hasAnyRole('ROLE_ADMIN')">
											<div class="col-sm-4 column">
												<div class="form-group">
													 ...
													 <form:select id="idForCourse" path="idForCourse"
														class="form-control">
														<form:option value=""><strong>Select Course</strong></form:option>
														<c:forEach var="course" items="${courses}"
															varStatus="status">
															<form:option value="${course.id}">${course.courseName}</form:option>
														</c:forEach>
										...
										<div class="col-sm-8 column">
											<div class="form-group">
												...
												<%-- <c:if test="${content.moduleId eq null}">
													<button id="submit" class="btn btn-large btn-primary"
														formaction="updateFile">Update Folder</button>
												</c:if> --%>
												...
												%>
												<button id="submit" class="btn btn-large btn-primary"
													formaction="addFile">Add File</button>
												<%
													}
												%>
